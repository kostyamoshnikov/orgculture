...
SECTIONS = [
  ...
  ("3. Категории обрабатываемых данных", [
    ...
    "Также автоматически обрабатываются обезличенные технические данные о посещениях сайта (файлы cookie, статистика переходов и действий на странице) через сервис Яндекс.Метрика, а также собственная статистика посещений Оператора (адрес страницы, источник перехода — без cookie, без сбора идентификаторов пользователя, только для внутренней аналитики) — подробнее в Соглашении об использовании cookie.",
  ]),
...
<body>
  ...
  <div class="meta">orgculture.ru · редакция от 24.09.2026</div>
  {sections_html}
...
subprocess.run([
    ...
    tmp_path, os.path.join(PARTS_DIR, "privacy-policy_ru.pdf")
])
os.remove(tmp_path)

print("site-pdf-parts/privacy-policy_ru.pdf written — на сайт: python3 build_bilingual_pdfs.py")
