...
SECTIONS = [
  ...
  ("2. Какие cookie используются", [
    ...
    "Отдельно от Яндекс.Метрики сайт собирает обезличенную статистику посещений через собственную инфраструктуру Оператора (без установки cookie — только адрес страницы и источник перехода, для внутренней статистики). Включается тем же согласием, что и Яндекс.Метрика, и отключается тем же способом.",
